 ...
 * 弹窗页脚里的按钮给一个宽度下限。
 *
 * 这些按钮都用 `:loading`，而 Element Plus 的加载态会在文字前插一个转圈图标 ——
 * 按钮于是变宽一下、加载完再缩回去。表格页那几个「刷新」按钮的同类抖动改成了
 * `:disabled`（那边有表格遮罩做反馈），但**弹窗里没有别的加载指示**：去掉转圈
 * 就成了「点了没反应」，比抖一下更糟。所以这里换个办法 —— 把宽度钉住，
 * 转圈加进来也撑不动它。顺带让各弹窗的「取消 / 保存」宽度一致。
 *
 * 只钉页脚：正文里那些 link 按钮（显示 / 复制）本来就窄，钉了反而难看。
 */
