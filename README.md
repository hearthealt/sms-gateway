...
- [短信采集规则](#短信采集规则)
- [消息转发（v2）](#消息转发v2)
- [远程指令（服务端 → 设备）](#远程指令服务端--设备)
- [外发短信（设备替你发）](#外发短信设备替你发)
- [故障告警](#故障告警)
- [配置说明](#配置说明)
- [设备端排查](#设备端排查)
- [安全注意事项](#安全注意事项)
...
### `POST /api/sms/send` — 用某台设备的号发一条短信

**唯一一个会产生费用的接口**，详见 [外发短信](#外发短信设备替你发)。
`deviceId` 省略时仅当服务器上只有一台设备才自动采用。

### `GET /api/sms/send/{id}` — 查外发状态

`PENDING` / `DISPATCHED` / `SENT` / `FAILED` / `CANCELLED` / `UNKNOWN`，见上面那一节。
...
### `GET /api/device/eventlog` — 本设备在服务端的运行日志

给「一键导出诊断包」用（`limit` 默认 200，服务端夹到 200，不分页）。接收号码**已打码**。
与 `GET /api/device/sms` 同一条规矩：设备读自己的数据，身份取自令牌。

### `POST /api/device/command/ack` — 远程指令的回执

```json
{ "results": [{ "id": 1024, "status": "DONE", "detail": "网关已停止" }] }
```

`status` 取 `DONE` / `FAILED` / `REJECTED`；`REJECTED` 是旧版本 App 遇到不认识的新指令
类型时用的。**重复回执是幂等的**，返回的 `accepted` 是「属于本设备且服务端认下」的条数 ——
设备据此停止重发。走独立端点而不是搭下一次心跳：回执必须立刻发，否则管理员点完
「停止网关」之后 60 秒内看到的还是「已下发」，会以为按钮没反应。
...
**这条通道还承载下行**（详见 [远程指令](#远程指令服务端--设备)）：

- 响应里的 `commands` 是要设备执行的远程指令（恒定出现，空为 `[]`）
- 响应里的 `outbound` 是要设备发出去的外发短信（探测心跳不带）
- 请求里的 `outboundResults` 是外发短信的执行结果（设备每次心跳都带上没确认的那些，服务端幂等）
- 请求里的 `commandProbe=true` 表示这是「网关已停止」时的低频探测心跳：
  服务端在它上面**只下发「启动网关」**，并且**完全不更新在线状态**
...
| POST | `/device/{deviceId}/command` | **下发远程指令**（`{type, argument}`） |
| GET | `/device/{deviceId}/command/list` | 该设备的指令记录 |
| POST | `/device/command/{id}/cancel` | 撤销一条还没结束的指令（路径不带 deviceId：id 全局唯一） |
| GET | `/outbound/list` | 外发短信记录（按设备/状态筛） |
| POST | `/outbound` | **发一条短信**（`{deviceId, phone, content}`，会产生费用） |
| POST | `/outbound/{id}/cancel` | 撤销一条**还没交给设备**的外发短信 |
| GET | `/alert/rule/list` | 告警规则列表 |
| GET | `/alert/rule/types` | 告警类型下拉（前端不硬编码映射） |
| POST | `/alert/rule` | 新建告警规则（至少配一个渠道） |
| PUT | `/alert/rule/{id}` | 修改告警规则 |
| DELETE | `/alert/rule/{id}` | 删除告警规则 |
| PUT | `/alert/rule/{id}/enabled` | 启停告警规则 |
...
在这之前这个协议是单向的：设备上报，服务端只能看。改一台设备的手机号、把它停掉，
都得有人走到那台手机前面。**远程指令就是那条下行通道。**

管理端在**设备详情页**里发，能做的五件事：

| 指令 | 它做什么 |
| --- | --- |
| 启动网关 / 停止网关 | 等同于在手机上按那个开关。启动会顺带把积压的短信传上去 |
| 修改本机号码 | 等同设置页里手填号码（卡槽记作「未知」——远程下发的号码不属于任何一张卡） |
| 触发一次重传 | 立刻把待上传的重试一遍，不等下一个 5 分钟周期 |
| 清理本地已上传记录 | 只删**已成功上传**的，未上传的不动 |
| 重新注册 | 用**本机已有的**身份重跑一次注册（同步版本号、对齐被人改乱的服务端记录） |

### 传输：搭心跳的便车，不新开长轮询

设备没有第二条常连通道（内网部署、没有 FCM），而它本来就每 30 秒问一次。
长轮询要多造一整条连接生命周期（hold 超时、重连退避、与在线判定冲突），
换来的只是 30 秒 → 约 0 秒的延迟改进，而这几条指令**没有一条是时间敏感的**。
所以指令塞在心跳响应里（`data.commands`），上限 5 条/次。

**代价是诚实写在界面上的**：设备在线时约 30 秒生效；网关已停止时最长 15 分钟。

### 回执丢了怎么办：`SENT` 会被重复下发，设备按 id 去重

```
PENDING ─下发→ SENT ─回执 DONE→ ACKED
                 ├─回执 FAILED/REJECTED→ FAILED
                 ├─下次 next_deliver_at 到点 → 再下发（attempts++）
                 └─attempts 用尽 / 过期 → EXPIRED
```

一条指令停在 `SENT`（回执丢在路上、设备离线）时，下个心跳会**再下发一次**；
设备按 `command id` 认出「这条我做过」，于是**不重复执行、只把上次的结论重发一遍回执**
（本地台账 `CommandAckStore` 记最近 20 条）。

不做「恰好一次投递」：那需要服务端的 claim/lease 协议，而设备侧每种指令本来就幂等
（起一个已在跑的服务、删一次已上传行都是空操作）。两台心跳并发捞到同一条 PENDING 时
**两个都会下发它，设备看到两次相同的 id、执行一次** —— 这不是 bug，是设计。

### 网关已停止时，靠一个 15 分钟的低频探测送达

这里有个死结：指令搭心跳下发，而用户停了网关**就没人发心跳** —— 于是「启动网关」
这条最需要在「已停」状态下送达的指令，恰好送不到。

解法是设备在停止期间跑一个 15 分钟的周期任务（`CommandProbeWorker`）发一种特殊心跳
（`commandProbe=true`）。服务端在它上面**只下发「启动网关」**，并且**完全不更新在线状态**
—— 否则后台会在用户明明按了停止之后，每 15 分钟把设备显示成在线一次。

探测只放「启动」一条是刻意的：用户停网关的意图是「这台机器冻结住，短信留在本地」，
从一个他看不见的后台任务里执行「清理已上传记录」是在**违背这个意图改本机状态**。

### 刷新一下认知：**不做「服务端下发新的重注册密钥」**

这是本次最要紧的一条设计决定。新增一条能把明文 `enrollSecret` 送到设备手上的通道，
会制造一次**跨越密钥轮换的权限提升**：

- `deviceToken = HMAC-SHA256(deviceId, app.secret.key)` 是确定性推导 ——
  拿到主密钥的人本来就能冒充任意设备（见安全注意事项第 2 条），这一点无法避免；
- 但**轮换主密钥会让所有已签发令牌立即失效**，那是现有的止损手段；
- 一旦设备能通过下行通道拿到新密钥，攻击者就可以在被清理前换一份新的，
  **主密钥轮换之后他仍然控制着那台设备** —— 恢复手段被指令本身废掉了。

第二条理由是这条指令的目的自相矛盾：恢复码存在的前提是「设备**丢了**密钥」，
而能收到下行指令的设备手里有一份**能用**的令牌。

所以 `RE_REGISTER` 落地的是安全版本（用本机已有身份重跑注册），
而**带任何参数都会被服务端拒绝**，报错里写全上面这段分析。
设备真丢了密钥时，唯一的路是管理员签发恢复码、由现场的人扫码 ——
**二维码必须进到手机的摄像头前，这是物理约束，不是设计缺陷。**

### 不做的事

任意 shell、下推配置 blob、OTA 更新、远程 logcat/读文件、远程发短信、远程重启/恢复出厂
（需要 Device Owner）、批量全员指令（每条指令至少写 2 行事件，一次 200 台的批量会把
「运行日志」页当天废掉；而且「清理本地记录」这类动作批量做是误操作的放大器）、
下发后临时提高心跳频率、经下行通道下发新密钥。

---

## 外发短信（设备替你发）

**这是全项目唯一一个会主动产生费用的功能。** 用某台设备的卡，把一条短信真的发出去。

三个入口：管理端「发送短信」页、对外 API `POST /api/sms/send`、以及未来的任何调用方。
**发出去不可撤回，按运营商计费。**

### 三道闸

1. **每台设备每日上限**（`outbound.daily-limit-per-device`，默认 20，`0` = 不限）。
   按**入队时刻**计数而不是发出时刻 —— 否则「今天入队一批、明天集中发出」会绕开它。
2. **绝不重发。** 这是它与远程指令最关键的差别：指令重发的代价是「设备可能多做一次」
   （而那些动作本来幂等），短信重发的代价是**真的又发一条出去、又计费一次**。
3. **「交给设备」与「设备发出」分开记**（`DISPATCHED` / `SENT`）。合成一个就会在回执
   丢的时候把「不知道发没发」显示成「已发出」。

### 回执丢了就照实说「结果未知」

回执随下一次心跳上报（不单开接口：这条短信已经离开手机了，晚几秒无所谓）。
交给设备超过 10 分钟还没回执的，由清理任务判成 `UNKNOWN`（**结果未知**）：
既不能说成功（没证据），也不能说失败（可能已经发出去了），更不能重发。
控制台照实显示，让人自己去对账。

设备那边：分段的短信**攒齐所有段**才给结论（前两段成功、第三段失败时整体是失败 ——
对方收到半截内容与没收到一样没用）；分段数由设备回报，**那个数字直接等于计费条数**。

### 设备端需要 `SEND_SMS`

这个权限**不在启动时批量申请**：应用的主职是收码，为一个可能永远用不到的能力在首次启动
就弹「发送短信」授权，只会让人怀疑它是干嘛的。

没有它时怎么办（**这个流程走过一遍，不是设想**）：

1. 控制台的记录会停在「发送失败」，说明里写着「本机未授予「发送短信」权限，请在那台手机上
   打开应用的自检页授权」
2. 在那台手机上打开应用 → **自检页** → 「发送短信权限」那一项 → 点「去授权」→ 系统弹框
3. 回到控制台，在那条失败的记录上点**「重发」**

权限那一项**只在没授予时才出现在自检页上** —— 那一页的目的是「为什么某个功能用不了」，
而不是把所有能力列一遍。（权限申请只能由界面发起：后台服务弹不出系统权限框。）

⚠️ 上架 Google Play 需要过短信权限政策审查（本项目侧载分发，但这一点要写在这里）。

### 失败了能重发，但「结果未知」的不能

| 状态 | 能不能重发 | 为什么 |
| --- | --- | --- |
| 发送失败 | ✅ 控制台那一行上有「重发」 | 设备**明确说了没发出去**（无信号、没权限、内容无效…），重发不会重复计费 |
| 结果未知 | ❌ 服务端会拒绝并说明原因 | 它**可能已经发出去了**，重发就是真的再发一条、再计费一次 —— 那条只能靠人去对账 |

这条界线是整个外发功能里最要紧的一处：重发的代价不是「多做一次动作」，而是**对方收到两条、
我们被计费两次**。
...
```bash
# 用 deviceId 指定哪台设备发
curl -X POST -H "Authorization: Bearer sk-xxxxxxxx" -H "Content-Type: application/json" \
  -d '{"deviceId":"android-xxx","phone":"13800138000","content":"提醒内容"}' \
  http://localhost:8080/api/sms/send

# 查状态（设备回执要等下一次心跳，约 30 秒）
curl -H "Authorization: Bearer sk-xxxxxxxx" http://localhost:8080/api/sms/send/12
```

`deviceId` 可以省略，但**只在服务器上恰好只有一台设备时**才自动采用；
有多台会明确要求指定。刻意**不按号码反查设备**：请求里的 `phone` 是**收信方**，
而设备是靠它自己的号码登记的，两者绝大多数时候不是同一个 ——
拿收信方的号码去猜「用哪台设备发」，一旦猜中（比如正好给设备自己的号发一条），
会用一个看起来正确的答案掩盖掉一次错误的调用。

对外响应里**不含** `deviceId` / `createdBy` / 分段数 / 设备回报的原始错误码 ——
与 `ClientSmsView` 同一条规矩：内部信息不进对外契约。

---

## 故障告警

设备离线、转发渠道连续失败、远程指令没执行成功 —— 这三件事原先只写进运行日志，
**没有人会主动去翻**。故障告警把它们推到已有的转发渠道上。

### 与转发规则**分表**，但**共用投递链路**

告警规则是独立的一张表（`alert_rule` + `alert_rule_channel`）：转发规则的匹配维度全是
短信的（发送方 / 关键词 / 接收号码），而告警的维度是「什么坏了 + 哪台设备」。
把 `sender_pattern` 临时当成「告警类型」用，就是让一列承载两种语义。

而**投递**复用 `notify_delivery`（加 `source_type` / `alert_type` / `subject_key` /
`alert_summary` 四列）：另起一张表 + 第二套调度器要复制退避、限流、卡死回收、
连续失败自动停用、脱敏与控制台页面约 400 行并发代码，两份必然分叉。
代价是三处必须按 `source_type` 分支的读取点，它们各有一条单测钉着。

### 三类告警，以及「离线」是另一套判据

| 类型 | 触发条件 |
| --- | --- |
| 设备离线 | **沉默** ≥ 10 分钟（`alert.offline-after-minutes`） |
| 转发渠道被自动停用 | 渠道连续失败达阈值自动停用时（排除刚死的那个渠道） |
| 远程指令失败 | 设备回执 FAILED/REJECTED，或指令过期未送达 |

⚠️ **告警的「离线」与设备列表上的「离线」不是一回事**，别混：

- 设备列表：心跳超过 **90 秒**就算离线（哪个方向都算）
- 告警：沉默超过 10 分钟，且**三个排除** —— 从没心跳过的不算（那是「还没启用」）、
  已被禁用的不算、**主动停机的不算**（否则管理员会收到一条自己刚按下的停止触发的告警）

最后那条还牵出 A1 的一处配套：远程停机被执行后，**服务端会自己写一次
`reported_offline_at`**。设备自己那次上报（`/api/device/offline`）是尽力而为的，
网络一断就发不出去；少了服务端这一手，那种停机会被判成「掉线」而触发告警。

### 防刷屏：两个 Redis 键，各管一件事

| 键 | TTL | 何时清 | 管什么 |
| --- | --- | --- | --- |
| `sms:alert:episode:{type}:{subject}` | 7 天 | **设备恢复在线时主动删** | 一次离线只告警一次 |
| `sms:alert:cooldown:{type}:{subject}` | `alert.cooldown-minutes`（60） | 自然过期，**恢复时不清** | 反复抖动时每冷却窗口最多一条 |

两个键都要拿到才发。效果：网络抖两分钟 → 一次都不发；离线 20 分钟恢复 → 一条；
接着又离线 → **不发**（故障期空着，但冷却没过期）；断断续续离线一整天 → 最多 24 条，
而不是几百条。**冷却键在恢复时不清，正是防抖的全部** —— 清了它等于每轮都发一条。

Redis 挂掉时**放行**：代价是可能重复告警，比「Redis 一挂、故障告警全部消失」轻得多，
而那恰恰是最需要告警的时刻。

### 静默时段是**推迟**，不是丢弃

落在静默时段内的告警，`next_retry_at` 设成时段结束时刻（复用调度器「限流推迟」那套语义），
控制台会诚实显示「待投递 + 下次重试 08:00」。一条「设备离线」早晨八点发出来仍然有用 ——
设备还离线着。时段可跨零点；`start == end` 有歧义，所以另有一个显式开关，
不靠相等性推断。

### 一键导出诊断包（Android）

设置页与自检页各有一个入口，把本地运行日志、队列状态、配置与自检结果、以及**服务端**的
本设备日志打成一份 txt 交给分享面板。把现场排查从「来回问十轮」变成「发一个文件」。

**脱敏是一条硬规则 + 一道会失败的断言**：

- **绝不导出**：`deviceToken` / `enrollSecret` / `enrollToken` / 锁屏 PIN 的哈希与盐、
  短信正文、验证码、**`localMessageId`**（它由「发送方 + 接收时刻 + 正文哈希」拼成，
  进文件等于换一种方式留痕）、**logcat**（debug 包的 OkHttp 日志里有 `Authorization: Bearer`
  与短信正文，打进去等于亲手把整份脱敏策略作废）。
- **完整导出**：设备标识（它不是凭据，而是本地日志与服务端日志之间唯一的连接键）、服务器地址。
- **打码导出**：手机号与发送方（与服务端 `PhoneUtil.mask` 同一口径）。
- 队列行打印**正文字节数**而不是正文。

写完还会**再验一遍**：把当前所有敏感值逐个在正文里搜，命中任何一个就拒绝出文件。
不靠「写的时候记得脱敏」（那是一种会随人而失效的保证），而失败是响亮的 ——
那本身就是「有人新加了一个字段却忘了脱敏」的信号。
...
### 系统升到 Android 15 之后可能看到什么

**现在不用管**（`targetSdk` 还是 34，那些限制只对目标 API 35+ 的应用生效），
但要知道它存在：Android 15 起，`dataSync` 类型的前台服务**每天只给 6 小时**，
而且**开机广播不能拉起**它。撞上之后的表现是「设备莫名不再上报，或后台一直显示离线」。

完整的影响分析、四个候选方案（推荐换成 `specialUse`）、以及**不用改 targetSdk 就能
提前验证**的 adb 命令，见 [`docs/android15-keepalive.md`](docs/android15-keepalive.md)。
...
14. **下发远程指令的权限边界，等于管理员账号的强度。** 谁能登进管理后台，谁就能停掉任何一台设备的网关、改它的号码、清它的本地记录。这与第 13 条是同一件事的两面：管理员账号的强度不是一个「能不能改配置」的问题，而是**能不能动到那些手机上的一切**。

15. **外发短信是要花钱、也受运营商约束的动作。** 服务端有每台设备每日上限（默认 20，`0` = 不限），但上限只是防呆，不是防滥用 —— 真正要防的是**调用方**（谁能拿到 API Key，谁就能用你的号发短信）。签发 API Key 时请按调用方隔离，别共用一把。另外 `SEND_SMS` 会触发 Google Play 的短信权限政策审查，本项目侧载分发不受影响，但要上架得先过那一关。

16. **「一键导出诊断包」生成的文件会离开设备。** 它经过系统分享面板进到某个聊天工具里，对面和云端都会存一份。为此有两条措施：**结构性排除**（正文、验证码、令牌、重注册密钥、`localMessageId`、logcat 一律不进文件）与**生成后自检**（把当前所有敏感值逐个在正文里搜，命中任何一个就拒绝出文件）。手机号与发送方只保留前 3 后 4。把这份文件发给别人之前，仍然值得自己扫一眼 —— 它包含设备标识与服务器内网地址。
