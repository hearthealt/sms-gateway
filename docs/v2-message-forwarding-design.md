...
> 状态：**已实施**（2026-09-20）　|　版本：v2.0
> 关联文档：[README](../README.md)
>
> 实施时的几处偏离，以代码为准：
>
> 1. **不做消息模板、不给验证码打码** —— 转发出去的就是短信原文 + 一行元信息。
>    数据库里没有 `template` / `mask_policy` 两列。这是一个明确的取舍：省掉一整块
>    配置面，代价是「群里都有谁」直接决定验证码的可见范围（管理端建渠道时提示一次）。
> 2. **§12 的四期合并实施**，十种渠道一次做完（除明确不做的三类）。
> 3. `GENERIC_WEBHOOK` 的 payload 不做可配模板，结构固定。
> 4. 出站 HTTP 用 JDK 自带的 `HttpClient`，未引入 Apache HttpClient。
> 5. **没有在消息前拼固定关键词**（§7.1 钉钉那条「需在转发内容前固定拼一个关键词」）。
>    拼了会污染所有渠道的消息，而钉钉的关键词填任何正文里出现过的词都行 ——
>    现在推荐填 `验证码`。见 [`notify-channel-setup.md`](notify-channel-setup.md) 第 3 节。
> 6. Telegram 的 `parameters.retry_after` 没有精确采用，用「429 至少等 30 秒」近似。
